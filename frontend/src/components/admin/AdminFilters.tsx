...
export const validateFilterDates = (startStr: string, endStr: string): string | null => {
...
export default function AdminFilters({
  ...
  const handleFieldChange = (key: keyof AdminFilterState, value: any) => {
    const updated = { ...filters, [key]: value };
    setFilters(updated);

    if (key === 'startDate' || key === 'endDate') {
      const err = validateFilterDates(updated.startDate, updated.endDate);
      setError(err);
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const err = validateFilterDates(filters.startDate, filters.endDate);
    if (err) {
      setError(err);
      return;
    }
  ...
  const companyOptions = companies
    ? [{ id: 'all', label: 'Tất cả công ty cứu hộ' }, ...companies.map((c) => ({ id: c._id, label: c.company_name }))]
    : [];

  const categoryOptions = categories
    ? [{ id: 'all', label: 'Tất cả dịch vụ' }, ...categories.map((c) => ({ id: c._id, label: c.name }))]
    : [];
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
          {companies && (
            <Select
              ...
              onChange={(e) => handleFieldChange('companyId', e.target.value || 'all')}
              options={companyOptions}
              placeholder="Tất cả công ty cứu hộ"
            />
          )}
